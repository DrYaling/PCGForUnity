...
void SetProcessPriority(std::string const& logChannel, uint32 affinity, bool highPriority)
{
    ...
    if (affinity > 0)
    {
        ...
        if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
        {
            ...
            if (!currentAffinity)
                LogFormat("Processors marked in UseProcessors bitmask (hex) %x are not accessible. Accessible processors bitmask (hex): %x", affinity, appAff);
            else if (SetProcessAffinityMask(hProcess, currentAffinity))
				LogFormat("Using processors (bitmask, hex): %x", currentAffinity);
            else
				LogFormat("Can't set used processors (hex): %x", currentAffinity);
        }
    }
    ...
    if (highPriority)
    {
        if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
			LogFormat("Process priority class set to HIGH");
        else
			LogFormat("Can't set process priority class.");
    }
